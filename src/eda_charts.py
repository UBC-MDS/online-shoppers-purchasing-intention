...
def chart_target_distribution(data, target_var, output_path):
    ...
    """
    ch_target_dist = alt.Chart(data).mark_bar().encode(
...
def chart_numeric_var_distribution(data, numeric_vars, output_path):
    ...
    """
    ch_numeric_vars_dist = alt.Chart(data).mark_bar().encode(
    ...
    ch_numeric_vars_dist.save(os.path.join(output_path, "chart_numeric_var_distribution.png"))
...
def chart_categorical_var_count(data, categorical_vars, output_path):
    ...
    """
    ch_categorical_vars_count = alt.Chart(data).mark_bar().encode(
    ...
    ch_categorical_vars_count.save(os.path.join(output_path, "chart_categorical_var_count.png"))
...
def main(input_path, output_path):
    ...
    chart_numeric_var_distribution(data=df, numeric_vars=numeric_cols, output_path=output_path)
    # Create chart to visualize categorical variables
    print(f'Creating chart for categorical variables')
    chart_categorical_var_count(data=df, categorical_vars=category_cols, output_path=output_path)
